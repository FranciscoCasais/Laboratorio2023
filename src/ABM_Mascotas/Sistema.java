...
public class Sistema {
    ...
    public String saludar(Mascota mascota,Persona duenio) {
        for(Mascota m:mascotas) {
            if(m.equals(mascota) && m.getDuenio().equals(duenio)) return m.saludo();
            else if(m.equals(mascota) && !(m instanceof Cantor) && !(m instanceof NoCantor)) return m.saludo().toUpperCase()+'!';
            else return null;
        }
        return null;
    }
