...
public class Cafetera {
    ...
    public Cafetera(double cantidadActual,double cantidadMaxima) {
        if(cantidadActual>cantidadMaxima) { this.cantidadActual=cantidadMaxima; }
    ...
    public void servirTaza(int capacidad) {
        if(cantidadActual>=capacidad) { cantidadActual-=capacidad;}
        else { cantidadActual=0; }
    }
    public void vaciarCafetera() { cantidadActual = 0; }
    public void agregarCafe(int cantidad) {
        if (cantidadActual+cantidad<=cantidadMaxima) { cantidadActual+=cantidad; }
        else { System.out.println("Error: cantidad máxima de la cafetera excedida."); }
    }
}